    ...
    protected void configure(HttpSecurity http) throws Exception {
    	http.csrf().disable().addFilterBefore(corsFilter(), SessionManagementFilter.class)
            .authorizeRequests()
                ...
                .antMatchers("/api/diachi/thanhpho").permitAll()
                .antMatchers("/api/diachi/quanhuyen").permitAll()
                .antMatchers("/api/diachi/thitran").permitAll()
                .antMatchers("/api/sanpham/trang").permitAll()
                .antMatchers("/api/img/them").permitAll()
                .antMatchers("/api/img/hinhanh").permitAll()
                .antMatchers("/api/hoadon/thanhtoan").hasAnyRole("MEMBER", "ADMIN")
                .antMatchers("/api/hoadon/chitiet").hasAnyRole("MEMBER", "ADMIN")
                .antMatchers("/api/hoadon/danhsach").hasAnyRole("MEMBER", "ADMIN")
                .antMatchers("/api/quanly/khachhang/danhsach").permitAll()
                .antMatchers("/api/khachhang/capnhat").hasAnyRole("MEMBER", "ADMIN")
                .antMatchers("/api/khachhang/chitiet").hasAnyRole("MEMBER", "ADMIN")
                .antMatchers("/api/quanly/sanpham/timkiem").permitAll()
                .antMatchers("/api/quanly/sanpham/them").permitAll()
                .antMatchers("/api/quanly/sanpham/xoa").permitAll()
                .antMatchers("/api/quanly/sanpham/capnhat").permitAll()
